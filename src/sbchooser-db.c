	 ...
	 const efi_secdb_data_t * const data, const size_t datasz)
{
	 ...
	 */
	//printf("old_certsp:%p *old_certsp:%p, n_certs:%zu, sizeof(cert_data_t):%zu\n",
	//       old_certsp, *old_certsp, n_certs, sizeof(cert_data_t));
...
free_secdb_info(sbchooser_context_t *ctx)
{
	...
	for (size_t i = 0; i < ctx->n_db_certs; i++) {
		cert_data_t *cert = ctx->db_certs[i];
	...
	for (size_t i = 0; i < ctx->n_dbx_certs; i++) {
		cert_data_t *cert = ctx->dbx_certs[i];

		if (cert->x509) {
			debug("free cert->x509:%p\n", cert->x509);
			X509_free(cert->x509);
			cert->x509 = NULL;
		}
		free(cert);
