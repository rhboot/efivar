...
struct digest_buffer {
	...
	uint8_t *digest;	// output buffer
	size_t digest_size;	// output buffer size
};
...
generate_authenticode_begin(digest_buffer_t **dbufs)
{
	...
	for (size_t i = 0; dbufs[i] != NULL; i++) {
		...
		memset(dbp->digest, 0, dbp->digest_size);
		...
		dbp->digest_size = EVP_MD_get_size(md);
		dbp->digest = calloc(1, dbp->digest_size);
		if (!dbp->digest) {
...
generate_authenticode_final(digest_buffer_t **dbufs)
{
	for (size_t i = 0; dbufs[i] != NULL; i++) {
		...
		EVP_DigestFinal(dbp->mdctx, dbp->digest, NULL);
		EVP_MD_CTX_free(dbp->mdctx);
		dbp->mdctx = NULL;
	}
	return 0;
...
generate_authenticode(pe_file_t *pe)
{
	...
	for (size_t i = 0; dbufs[i] != NULL; i++) {
		...
		if (!strcmp(dbp->algorithm, "SHA256")) {
			pe->sha256.data = dbp->digest;
			pe->sha256.datasz = dbp->digest_size;
		} else if (!strcmp(dbp->algorithm, "SHA384")) {
			pe->sha384.data = dbp->digest;
			pe->sha384.datasz = dbp->digest_size;
		} else if (!strcmp(dbp->algorithm, "SHA512")) {
			pe->sha512.data = dbp->digest;
			pe->sha512.datasz = dbp->digest_size;
		}
		EVP_MD_CTX_free(dbp->mdctx);
	}
	...
	if (efi_get_verbose() >= DEBUG_LEVEL) {
		for (size_t i = 0; dbufs[i] != NULL; i++) {
			...
			fprintf(stderr, "%s:", dbp->algorithm);
			for (size_t j = 0; j < dbp->digest_size; j++)
				fprintf(stderr, "%02x", dbp->digest[j]);
			fprintf(stderr, "\n");
		}
	}
