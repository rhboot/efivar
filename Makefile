...
install :
	@for x in $(SUBDIRS) ; do make -C $${x} TOPDIR=$(TOPDIR) SRCDIR=$(TOPDIR)/$@/ ARCH=$(ARCH) VERSION=$(VERSION) INSTALLROOT=$(INSTALLROOT) INCDIR=$(INCDIR) BINDIR=$(BINDIR) LIBDIR=$(LIBDIR) PCDIR=$(PCDIR) $@ ; done
